...
public interface ReservationRepository extends JpaRepository<Reservation, String> {
    ...
    @Query("SELECT COUNT(r) FROM Reservation r WHERE r.userId = :userId " +
           ...
           "AND r.status IN :statuses")
    long countReservationsByUserAndDate(
        @Param("userId") String userId,
    ...
    List<Reservation> findByUserIdAndStatusInOrderByStartTimeDesc(
        String userId, 
        List<String> statuses
    );

    // Find reservations by slot
    List<Reservation> findBySlotIdAndStatusInOrderByStartTimeAsc(
        String slotId, 
        List<String> statuses
    );
    ...
    @Query("SELECT r FROM Reservation r WHERE r.startTime BETWEEN :now AND :futureTime " +
